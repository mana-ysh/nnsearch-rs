...
impl NavigableSmallWorldGraph {
    fn approx_knn_search(&self, query: &VectorNode, k: usize) -> Vec<usize> {
        ...
        // FIXME: need many improvements :)
        // The algorithm here is based on https://publications.hse.ru/mirror/pubs/share/folder/x5p6h7thif/direct/128296059
        let mut rng = rand::thread_rng();
        let mut dist_memo: HashMap<usize, f32> = HashMap::new();
        let mut candidates: Vec<usize> = vec![];
        let mut visited = HashSet::new();
        let mut result: Vec<usize> = vec![];
        let ids: Vec<&usize> = self.id2node.iter().map(|(k, v)| k).collect();
        for i in 0..self.trial {
            candidates.push(ids.choose(&mut rng).unwrap().clone().clone());
            let mut temp_res = HashSet::new();
            loop {
                // TODO: sorting whole elements every time is inefficient maybe.
                // FIXME: 'get or insert' is better if possible.
                for &cand in &candidates {
                    if !dist_memo.contains_key(&cand) {
                        dist_memo.insert(cand, calcurate_distance(&self.distance_type, &query.vec, &self.get_node(&cand).unwrap().vec));
                    }
                }
                candidates.sort_by(|&a, &b| dist_memo.get(&a).unwrap().partial_cmp(dist_memo.get(&b).unwrap()).unwrap());
                let c = candidates.get(0).unwrap();
                // FIXME: inefficient
                if result.len() >= k {
                    result.sort_by(|&a, &b| dist_memo.get(&a).unwrap().partial_cmp(dist_memo.get(&b).unwrap()).unwrap());
                    if dist_memo.get(result.get(k-1).unwrap()).unwrap() <= dist_memo.get(c).unwrap() {
                        break
                    }
                }
                for &e in self.id2adjacency_ids.get(&c).unwrap_or(&vec![]) {
                    if !visited.contains(&e) {
                        visited.insert(e.clone());
                        candidates.push(e.clone());
                        temp_res.insert(e.clone());
                    }
                }
                for &id in &temp_res {
                    if !dist_memo.contains_key(&id) {
                        dist_memo.insert(id.clone(), calcurate_distance(&self.distance_type, &query.vec, &self.get_node(&id).unwrap().vec));
                    }
                    result.push(id.clone().clone());
                }
            }
        }
        result.sort_by(|&a, &b| dist_memo.get(&a).unwrap().partial_cmp(dist_memo.get(&b).unwrap()).unwrap());
...
impl GraphOperator for NavigableSmallWorldGraph {
    fn add_node(&mut self, node: VectorNode) -> Result<(), ()> {
        ...
        // connect nn -> node
        nn_ids.iter().for_each(|nn_id| self.id2adjacency_ids.get_mut(&node.id).unwrap().push(nn_id.clone()));
        self.id2node.insert(node.id, node);
        Ok(())
    }
    fn get_node(&self, id: &usize) -> Option<&VectorNode> {
